...
export class Game {
  ...
  private updateProjectiles(dt: number) {
    ...
    for (const projectile of this.projectiles) {
      ...
      if (d <= step + 8) {
        if (target?.alive && this.canResolveProjectileHit(projectile, target, source)) {
        ...
        const next = { x: previous.x + (dx / d) * step, y: previous.y + (dy / d) * step };
        if (this.segmentBlocked(previous, next)) continue;
        projectile.pos = next;
