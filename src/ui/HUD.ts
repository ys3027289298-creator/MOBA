...
import { itemSlotState } from '../engine/items';
...
export class HUD {
  ...
  update() {
    ...
    const itemLabels = player.items.map((id, index) => {
      ...
      if (!item) return `${index + 1}.未知装备`;
      if (!item.active) return `${index + 1}.${item.name}[被动]`;
      const slot = itemSlotState(player, id, model.result === 'running');
      const state = slot.state === 'cooldown' ? `冷却${slot.remaining.toFixed(0)}s`
        : slot.state === 'ready' ? '可用' : '不可用';
